...
class Base_Agent(object):
    ...
    def __init__(self, config):
        ...
        self.environment_title = self.environment.envName #self.get_environment_title()
        self.action_types = "DISCRETE" if self.environment.action_space.dtype == np.int64 else "CONTINUOUS"
        self.action_size = len(self.environment.action_space) #int(self.get_action_size())
        self.config.action_size = self.action_size #self.action_size

        self.lowest_possible_episode_score = self.get_lowest_possible_episode_score()

        self.state_size =  int(self.get_state_size())
        self.hyperparameters = config.hyperparameters
        self.average_score_required_to_win = 50 #self.get_score_required_to_win()
...
#        self.environment.seed(self.config.seed)
    ...
    def run_n_episodes(self, num_episodes=None, show_whether_achieved_goal=True, save_and_print_results=True):
        ...
        return self.game_full_episode_scores,self.game_full_episode_steps, self.rolling_results, time_taken

    def conduct_action(self, action):
        """Conducts an action in the environment"""
        self.next_state, self.reward, self.done, _ = self.environment.step(action)
        self.total_episode_score_so_far += self.reward
        self.total_episode_steps_so_far += 1
    ...
    def save_result(self):
        ...
        self.game_full_episode_steps.append(self.total_episode_steps_so_far)
    ...
    def print_rolling_result(self):
        ...
        text = """"\r Episode {0}, Score: {3: .2f}, Steps: {5}, Max score seen: {4: .2f}, Rolling score: {1: .2f}, Max rolling score seen: {2: .2f}"""
        sys.stdout.write(text.format(len(self.game_full_episode_scores), self.rolling_results[-1], self.max_rolling_score_seen,
                                     self.game_full_episode_scores[-1], self.max_episode_score_seen, self.game_full_episode_steps[-1]))
        sys.stdout.flush()

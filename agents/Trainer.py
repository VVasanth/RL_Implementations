...
class Trainer(object):
    ...
    def run_games_for_agent(self, agent_number, agent_class):
        ...
        for run in range(self.config.runs_per_agent):
            ...
            game_scores, game_steps, rolling_scores, time_taken = agent.run_n_episodes()
            print("Time taken: {}".format(time_taken), flush=True)
            self.print_two_empty_lines()
            agent_results.append([list(range(1, len(game_scores)+1)) ,game_scores, rolling_scores, game_steps, time_taken])
    ...
    def get_y_limits(self, results):
        ...
        for result in results:
            ...
            if temp_min < min_result:
                min_result = temp_min
        return min_result * 0.8, max_result * 1.2
